            ...
            <div class="col-md-6">
                ...
                <x-forms.group for="union_license-{{$level->id++}}" label="مجوز تاسیس اتحاد">
                    <x-forms.select wire:model="union_license" id="union_license-{{$level->id++}}">
                        <option @selected($level->union_licence) value="1">بله</option>
                        <option @selected($level->union_licence) value="0">خیر</option>
                    </x-forms.select>
                    @error('union_license')
                        <span class="form-text text-danger">{{ $message }}</span>
                    @enderror
                ...
                <x-forms.group for="observing_license-{{$level->id++}}" label="مجوز بازرسی">
                    <x-forms.select wire:model="observing_license" id="observing_license-{{$level->id++}}">
                        <option @selected($level->observing_license) value="1">بله</option>
                        <option @selected($level->observing_license) value="0">خیر</option>
                    </x-forms.select>
                ...
                <x-forms.group for="gate_license-{{$level->id++}}" label="مجوز دروازه">
                    <x-forms.select wire:model="gate_license" id="gate_license-{{$level->id++}}">
                        <option @selected($level->gate_license)  value="1">بله</option>
                        <option @selected($level->gate_license)  value="0">خیر</option>
                    </x-forms.select>
                    @error('gate_license')
                        <span class="form-text text-danger">{{ $message }}</span>
                    @enderror
            ...
            <div class="col-md-6">
                ...
                <x-forms.group for="lawyer_license-{{$level->id++}}" label="مجوز وکالت">
                    <x-forms.select wire:model="lawyer_license" id="lawyer_license-{{$level->id++}}">
                        <option @selected($level->lawyer_license) value="1">بله</option>
                        <option @selected($level->lawyer_license) value="0">خیر</option>
                    </x-forms.select>
                ...
                <x-forms.group for="city_counsile_entry-{{$level->id++}}" label="ورود به شورای شهر">
                    <x-forms.select wire:model="city_counsile_entry" id="city_counsile_entry-{{$level->id++}}">
                        <option @selected($level->city_counsile_entry) value="1">بله</option>
                        <option @selected($level->city_counsile_entry) value="0">خیر</option>
                    </x-forms.select>
                ...
                <x-forms.group for="judge_entry-{{$level->id++}}" label="ورود به قضاوت">
                    <x-forms.select wire:model="judge_entry" id="judge_entry-{{$level->id++}}">
                        <option @selected($level->judge_entry) value="0">انتخاب کنید</option>
                        <option @selected($level->judge_entry) value="1">دادگاه عمومی</option>
                        <option @selected($level->judge_entry) value="2">دادگاه کیفری</option>
                    </x-forms.select>
                    @error('judge_entry')
                        <span class="form-text text-danger">{{ $message }}</span>
                    @enderror
