          ...
          <template #footer>
            <div class="flex gap-2 p-4 border-t border-[var(--theme-border)] bg-[var(--theme-bg-glass)]">
              <Button
                ...
                rounded="full"
                full-width
                class="!gap-0 justify-center !py-2.5"
                title="مشاهده"
                aria-label="مشاهده"
    ...
    >
      <div v-if="selectedTicket" class="space-y-4">
        ...
        <div v-if="selectedTicket.status != 1" class="space-y-4">
          ...
          <FileInput
            v-model="responseAttachment"
            label="پیوست"
            accept=".pdf,.png,.jpeg,.jpg"
            placeholder="فایل پیوست (اختیاری)"
            helper-text="PDF یا تصویر"
            :error="fieldError(errors.attachment)"
            :clearable="true"
            @change="onResponseAttachmentChange"
          />
        </div>
      </div>
